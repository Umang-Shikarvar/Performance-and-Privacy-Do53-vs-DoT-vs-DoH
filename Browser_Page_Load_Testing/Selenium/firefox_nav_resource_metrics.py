# """
# firefox_nav_har_combined_profile.py
# -------------------------------------------------
# Combines Navigation Timing API metrics + Selenium-Wire HAR-like data
# and uses your real Firefox profile to preserve proxy/DNS settings.
...
# Collects:
# - DNS, TCP, TLS, DOM, and load timings (Navigation Timing API)
# - Number of HTTP(S) requests
# - Total bytes in/out (from response headers)
# - Count of 4xx / 5xx status codes
# -------------------------------------------------
# """
...
# # ------------- Configuration -------------
# URLS_FILE = "top_50.txt"
# OUTPUT_CSV = "UDP_nav_har_metrics.csv"
# RUNS_PER_URL = 1

# # Path to your actual Firefox profile (found from about:profiles)
...
# # firefox_options.add_argument("-headless")  # optional if you want to hide window

# # Load your existing Firefox profile (inherits proxy, DoH, certs, etc.)
# profile = FirefoxProfile(PROFILE_PATH)

# # You can still tweak minor preferences if needed:
# profile.set_preference("browser.cache.disk.enable", False)
# profile.set_preference("browser.cache.memory.enable", False)
# profile.set_preference("browser.cache.offline.enable", False)
# profile.set_preference("network.http.use-cache", False)

# # If you want to *force* your SOCKS proxy through code (optional override)
...
# # ------------- Launch Driver -------------
# # Attach your real Firefox profile to the Options object
# firefox_options.profile = profile

# # Then launch the driver (no firefox_profile arg)
# driver = webdriver.Firefox(
#     seleniumwire_options={},
#     options=firefox_options
# )

# print("\n[Firefox profile in use]:", driver.firefox_profile.path)

# results = []

# # ------------- Helper Function -------------
# def get_nav_metrics():
#     """Extract Navigation Timing API metrics for the main page load."""
...
#         tls_ms: (p.secureConnectionStart > 0) ? (p.connectEnd - p.secureConnectionStart) : 0,
#         request_ms: p.responseStart - p.requestStart,
#         response_ms: p.responseEnd - p.responseStart,
#         dom_content_loaded_ms: p.domContentLoadedEventEnd - p.startTime,
#         load_event_ms: p.loadEventEnd - p.startTime,
#         total_ms: p.duration
#     };
#     """)
...
#         # --- HAR-like data (requests summary) ---
#         reqs = [r for r in driver.requests if hasattr(r, "response") and r.response]
...
#         for r in reqs:
#             try:
#                 # Estimate bytes in from Content-Length header
#                 size = int(r.response.headers.get("Content-Length", 0))
#                 total_bytes_in += max(size, 0)

#                 # Estimate bytes out (roughly request + headers)
#                 req_size = len(r.url) + sum(len(k) + len(v) for k, v in r.headers.items())
#                 total_bytes_out += req_size

#                 code = int(r.response.status_code)
#                 if 400 <= code < 500:
#                     status_4xx += 1
#                 elif 500 <= code < 600:
#                     status_5xx += 1
...
#         # --- Combine metrics ---
#         def safe(k): return round(nav.get(k, 0), 2) if k in nav else 0
#         row = {
...
#             "dns_ms": safe("dns_ms"),                 # DNS resolution time
#             "tcp_ms": safe("tcp_ms"),                 # TCP connection time
#             "tls_ms": safe("tls_ms"),                 # TLS handshake time (HTTPS)
#             "request_ms": safe("request_ms"),         # Time from request start → first byte
#             "response_ms": safe("response_ms"),       # Time to receive full response
#             "dom_content_loaded_ms": safe("dom_content_loaded_ms"),  # DOM loaded
#             "load_event_ms": safe("load_event_ms"),   # Full load
#             "total_ms": safe("total_ms"),             # Total navigation time
#             # HAR-like metrics
...
# print(f"\n✅ Combined metrics saved to {OUTPUT_CSV}")
