...
# import resolver utilities from utils.py (make sure utils.py is in same dir)
from utils import perform_iterative_resolution, save_log_json

# =====================================================
#              TLS 1.2 SERVER CONFIG
# =====================================================
HOST = "0.0.0.0"
PORT = 8853
RT_HANDSHAKE = 22
...
def handle_connection(conn, addr):
    ...
    try:
        ...
        if ct != RT_HANDSHAKE or data is None:
            ...
            conn.close(); return
        # preserve canonical JSON bytes in transcript (client uses canonical form)
        ...
        try:
            ...
            print("Failed to decrypt client Finished:", e)
            conn.close(); return
        ...
        print("[*] TLS handshake complete (server)")

        # =====================================================
        #              DNS QUERY HANDLING (RESOLVER)
        # =====================================================
        ...
        try:
            ...
            print("Failed to decrypt application data:", e)
            conn.close(); return

        # parse query for logging
        try:
        ...
        # Call the utils resolver (returns response, logs, duration_ms, qname)
        try:
            response, details, duration_ms, queried_name = perform_iterative_resolution(plain)
        except TypeError:
            # fallback if different signature
            response, details, duration_ms, queried_name = perform_iterative_resolution(plain, is_subquery=False)

        status = "SUCCESS" if response else "FAILED"

        # If resolver returned None, create SERVFAIL reply
        if not response:
            try:
                req = DNSRecord.parse(plain)
        ...
        # Save enriched log (include client IP)
        record = {
        ...
        # Encrypt + send DNS response over TLS
        resp_nonce = make_nonce(server_iv, server_seq)
        resp_ct = server_aes.encrypt(resp_nonce, reply_bytes, None)
...
def server_main():
    ...
    s.bind((HOST, PORT))
    s.listen(5)
    print("TLS1.2-like DNS resolver server listening on", HOST, PORT)
    while True:
        c, a = s.accept()
